...
Thresholds thresh{40, 180, 40, 255, 50, 255}; // 初始阈值
...
int main() {
    ...
    while(1) {
        ...
        Scalar orange_low(thresh.H_low, thresh.S_low, thresh.V_low) , orange_high(thresh.H_high, 255, 255);
        ...
        dilate(bin, bin, kernel); // 膨胀
        erode(bin, bin, kernel); // 腐蚀
        ...
        Rect rec = boundingRect(Max_contour);
        rectangle(rst, rec, Scalar(0,255,0), 2);
        putText(rst, "Basketball", Point(rec.x, rec.y-10), FONT_HERSHEY_SIMPLEX, 0.9, Scalar(0,255,0), 2);  
